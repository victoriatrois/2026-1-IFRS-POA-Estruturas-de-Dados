...
TArvore *insere_binario_alfabetico(TArvore *a, int matricula, char nome[30]) {
  if (a == NULL) {
    a = Cria(NULL, matricula, nome, NULL);
  } else if (strcmp(nome, a->nome) < 0) {
    a->esq = insere_binario_alfabetico(a->esq, matricula, nome);
  } else if (strcmp(nome, a->nome) > 0) {
    a->dir = insere_binario_alfabetico(a->dir, matricula, nome);
  }
  return a;
}

TArvore *busca_binaria_por_nome(TArvore *a, char nome[30], int *comparacoesFeitas) {
  if (a == NULL) {
    return NULL;
  }
  ...
  if (nome == a->nome) {
    ...
    return busca_binaria(a->esq, nome, comparacoesFeitas);
  } else {
    return busca_binaria(a->dir, nome, comparacoesFeitas);
  }
}